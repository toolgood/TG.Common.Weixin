...
namespace TG.Common.Weixin.Mp
{
    ...
    /// 高级群发接口
    /// </summary>
    public class GroupMessageApi : MpApi
    {
        public GroupMessageApi(WxMpApi api) : base(api) { }
        /// <summary>
        ...
        /// <param name="timeOut">代理请求超时时间（毫秒）</param>
        /// <returns></returns>
        public SendResult SendGroupMessagePreview(GroupMessageType type, string value, string openId, string wxName = null, int timeOut = Config.TIME_OUT)
        {
            var accessToken = _api.GetAccessToken();
        ...
        /// <returns></returns>
        public SendResult WxCardGroupMessagePreview(string cardId, string code, string openId, string wxName, string timestamp, string signature, int timeOut = Config.TIME_OUT)
        {
            var accessToken = _api.GetAccessToken();
            string url = string.Format("https://api.weixin.qq.com/cgi-bin/message/mass/preview?access_token={0}", accessToken);
        ...
        /// <param name="timeOut">代理请求超时时间（毫秒）</param>
        /// <returns></returns>
        public GetSendResult GetGroupMessageResult(string msgId, int timeOut = Config.TIME_OUT)
        {
            var accessToken = _api.GetAccessToken();
        ...
        /// <param name="timeOut"></param>
        /// <returns></returns>
        public VideoMediaIdResult GetVideoMediaIdResult(string mediaId, string title, string description, int timeOut = Config.TIME_OUT)
        {
            var accessToken = _api.GetAccessToken();
...
namespace TG.Common.Weixin
{
    ...
    {
        GroupMessageApi _GroupMessageApi;
        /// <summary>
        /// 自定义菜单管理
        /// </summary>
        public GroupMessageApi GroupMessageApi
        {
            get
            {
                if (_GroupMessageApi==null)
                {
                    _GroupMessageApi= new GroupMessageApi(this);
                }
                return _GroupMessageApi;
            }
        }
